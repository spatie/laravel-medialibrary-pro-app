...
">
    ...
    @if($attributes["loading"])
        ...
        <div class="absolute inset-0 px-3 flex items-center justify-center rounded-full shadow">
    ...
    <div class="absolute w-full bottom-0">
        <div class="relative rounded-full text-xs text-center">
            <div class="absolute rounded-sm inset-0 opacity-75 shadow-md
            {{ $attributes["hover"] ? ($attributes["not-allowed"] ? 'bg-red-400' : 'bg-blue-400') : '' }}
            {{ $attributes["approaching"] ? ($attributes["not-allowed"] ? 'bg-red-400' : 'bg-blue-400') : '' }}
            {{ $attributes["server-error"] ? 'bg-red-500' : '' }}
            {{ $attributes["loading"] ? 'bg-gray-800' : '' }}
            {{ $attributes["empty"] ? '' :'bg-gray-800' }}
            "></div>
            <div class="relative px-2 py-1 {{ $attributes["empty"] ? 'text-gray-600' : 'text-white' }}">
                ...
                @if($attributes["maxSize"])
                    <span class="opacity-50">•</span> Max. {{ $attributes["maxSize"] }}
                @endif
